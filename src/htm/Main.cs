...
namespace thememan
{
    public partial class HTM : Form
    {
        ...
        public HTM ()
        {
            ...
            {
                xchatusdir = Path.Combine(portableDir, "config\\");

                if (!Directory.Exists(xchatusdir))
                {
                    MessageBox.Show("XCHATUS installation not found!\nCheck your .\\config folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Environment.Exit(1);
                }
            }
            else
            {
                 ...
                 * Windows: %APPDATA%
                 * Unix: ~/.config
                 * Windows is case-insensitive so 'xchatus' should be fine for both
                 */
                xchatusdir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "xchatus");

                if (!Directory.Exists(xchatusdir))
                {
                    if (RunningOnWindows())
                    {
                        MessageBox.Show("XCHATUS installation not found!\nCheck your %APPDATA%\\XCHATUS folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);    
                    }
                    else
                    {
                        MessageBox.Show("XCHATUS installation not found!\nCheck your ~/.config/xchatus folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
            ...
            themedir = Path.Combine(xchatusdir, "themes");
            ListThemes();
        ...
        {
            DialogResult result = MessageBox.Show("XCHATUS must be closed and this will overwrite your current theme!\n\nDo you wish to continue?", "Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (result == DialogResult.OK)
            {
                File.Copy(Path.Combine(themedir, themelist.SelectedItem.ToString(), "colors.conf"), Path.Combine(xchatusdir, "colors.conf"), true);
                if (File.Exists(Path.Combine(themedir, themelist.SelectedItem.ToString(), "pevents.conf")))
                {
                    File.Copy(Path.Combine(themedir, themelist.SelectedItem.ToString(), "pevents.conf"), Path.Combine(xchatusdir, "pevents.conf"), true);
                }
                else if (File.Exists(Path.Combine(xchatusdir, "pevents.conf")))
                {
                    File.Delete(Path.Combine(xchatusdir, "pevents.conf"));
                }
            }
        }
        ...
        private void importbutton_Click_1(object sender, EventArgs e)
        {
            ...
            importDialog.Filter = "XCHATUS Theme Files|*.hct";
