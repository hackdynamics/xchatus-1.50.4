/* dbus-client.c - XCHATUS command-line options for D-Bus
...
#define DBUS_REMOTE_PATH "/org/xchatus/Remote"
#define DBUS_REMOTE_INTERFACE "org.xchatus.plugin"
 ...
 */
{
	...
	/* Checks if XCHATUS is already running */
	...
	if (!ret)
	{
		...
		xchatus_running = FALSE;
	}
	else
	{
		GVariant *child = g_variant_get_child_value (ret, 0);
		xchatus_running = g_variant_get_boolean (child);
		g_variant_unref (ret);
	...
	if (!remote_object)
	{
		write_error("Failed to connect to XCHATUS", &error);
		g_object_unref (connection);
		exit (0);
	}
