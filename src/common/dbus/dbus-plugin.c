/* dbus-plugin.c - xchatus plugin for remote access using D-Bus
...
#define DBUS_OBJECT_PATH "/org/xchatus"
...
static guint		context_list_find_id		(xchatus_context *context);
static xchatus_context*	context_list_find_context	(guint id);
...
{
	xchatus_list_free (ph, (xchatus_list*)data);
}
...
remote_object_finalize (GObject *obj)
{
	...
	xchatus_plugingui_remove (ph, self->handle);
...
remote_object_init (RemoteObject *obj)
{
	...
	obj->last_hook_id = 0;
	obj->last_list_id = 0;
	obj->context = xchatus_get_context (ph);
}
...
remote_object_class_init (RemoteObjectClass *klass)
{
	...
	signals[SERVER_SIGNAL] =
		g_signal_new ("server_signal",
			      ...
			      0,
			      NULL, NULL,
			      _xchatus_marshal_VOID__POINTER_POINTER_UINT_UINT,
			      G_TYPE_NONE,
		       ...
		       DBusGMethodInvocation *context)
{
	...
	remote_object->handle = xchatus_plugingui_add (ph,
...
remote_object_command (RemoteObject *obj,
		       ...
		       GError **error)
{
	if (xchatus_set_context (ph, obj->context)) {
		xchatus_command (ph, command);
	}
	return TRUE;
}
			    ...
			    GError **error)
{
	...
	context = xchatus_find_context (ph, server, channel);
			...
			GError **error)
{
	...
	*ret_info = g_strdup (xchatus_get_info (ph, id));
	return TRUE;
}
			 ...
			 GError **error)
{
	...
	*ret_type = xchatus_get_prefs (ph, name, &str, ret_int);
		...
		void *userdata)
{
	...
	info->obj->context = xchatus_get_context (ph);
			    ...
			    GError **error)
{
	...
	info->hook = xchatus_hook_command (ph,
					 name,
					 priority,
					 command_hook_cb,
					 help_text,
			   ...
			   GError **error)
{
	...
	info->return_value = return_value;
	info->id = ++obj->last_hook_id;
	info->hook = xchatus_hook_server (ph,
					name,
					priority,
					server_hook_cb,
					info);
			  ...
			  GError **error)
{
	...
	info->return_value = return_value;
	info->id = ++obj->last_hook_id;
	info->hook = xchatus_hook_print (ph,
				       name,
				       priority,
				       print_hook_cb,
				       info);
			...
			GError **error)
{
	...
	xlist = xchatus_list_get (ph, name);
	if (xlist == NULL) {
		*ret_id = 0;
		return TRUE;
	}
			 ...
			 GError **error)
{
	...
	*ret = xchatus_list_next (ph, xlist);
			...
			GError **error)
{
	...
	xlist = g_hash_table_lookup (obj->lists, &id);
	if (xlist == NULL && !xchatus_set_context (ph, obj->context)) {
		*ret_str = NULL;
		return TRUE;
	}
	...
	*ret_str = g_strdup (xchatus_list_str (ph, xlist, name));
			...
			GError **error)
{
	...
	if (g_str_equal (name, "context")) {
		xchatus_context *context;
		context = (xchatus_context*)xchatus_list_str (ph, xlist, name);
		*ret_int = context_list_find_id (context);
	} else {
		*ret_int = xchatus_list_int (ph, xlist, name);
	}
			 ...
			 GError **error)
{
	...
	*ret_time = xchatus_list_time (ph, xlist, name);
...
remote_object_list_fields (RemoteObject *obj,
			   ...
			   GError **error)
{
	*ret = g_strdupv ((char**)xchatus_list_fields (ph, name));
	if (*ret == NULL) {
		*ret = g_new0 (char*, 1);
	}
	return TRUE;
			  ...
			  GError **error)
{
	...
	*ret = xchatus_set_context (ph, obj->context);
	if (*ret) {
		*ret = xchatus_emit_print (ph, event_name, argv[0], argv[1],
							 argv[2], argv[3]);
	}
...
remote_object_nickcmp (RemoteObject *obj,
		       ...
		       GError **error)
{
	xchatus_set_context (ph, obj->context);
	*ret = xchatus_nickcmp (ph, nick1, nick2);
	return TRUE;
}
...
remote_object_strip (RemoteObject *obj,
		     ...
		     GError **error)
{
	*ret_str = xchatus_strip (ph, str, len, flag);
	return TRUE;
}
...
init_dbus (void)
{
	...
	if (connection == NULL) {
		xchatus_printf (ph, _("Couldn't connect to session bus: %s\n"),
			      error->message);
		g_error_free (error);
		return FALSE;
	}
	...
	if (!dbus_g_proxy_call (proxy, "RequestName", &error,
				...
				G_TYPE_INVALID)) {
		xchatus_printf (ph, _("Failed to acquire %s: %s\n"),
			      DBUS_SERVICE,
			      error->message);
		g_error_free (error);
...
static guint
context_list_find_id (xchatus_context *context)
{
	GList *l = NULL;
		 ...
		 void *userdata)
{
	...
	info->context = xchatus_get_context (ph);
...
int
dbus_plugin_init (xchatus_plugin *plugin_handle,
		  char **plugin_name,
		  char **plugin_desc,
		  char **plugin_version,
		  char *arg)
{
	...
	*plugin_version = PVERSION;
	...
	if (init_dbus()) {
		/*xchatus_printf (ph, _("%s loaded successfully!\n"), PNAME);*/
		...
		xchatus_hook_print (ph, "Open Context",
				  HEXCHAT_PRI_NORM,
				  open_context_cb,
				  NULL);

		xchatus_hook_print (ph, "Close Context",
				  HEXCHAT_PRI_NORM,
				  close_context_cb,
				  NULL);

		xchatus_hook_command (ph, "unload",
				    HEXCHAT_PRI_HIGHEST,
