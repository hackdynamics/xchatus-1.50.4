...
#include "text.h"
#define PLUGIN_C
typedef struct session xchatus_context;
...
struct _xchatus_hook
{
	xchatus_plugin *pl;	/* the plugin to which it belongs */
	char *name;			/* "xdcc" */
...
typedef int (xchatus_cmd_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (xchatus_serv_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (xchatus_print_cb) (char *word[], void *user_data);
typedef int (xchatus_serv_attrs_cb) (char *word[], char *word_eol[], xchatus_event_attrs *attrs, void *user_data);
typedef int (xchatus_print_attrs_cb) (char *word[], xchatus_event_attrs *attrs, void *user_data);
typedef int (xchatus_fd_cb) (int fd, int flags, void *user_data);
typedef int (xchatus_timer_cb) (void *user_data);
typedef int (xchatus_init_func) (xchatus_plugin *, char **, char **, char **, char *);
typedef int (xchatus_deinit_func) (xchatus_plugin *);
...
static int
plugin_free (xchatus_plugin *pl, int do_deinit, int allow_refuse)
{
	GSList *list, *next;
	xchatus_hook *hook;
	xchatus_deinit_func *deinit_func;

	/* fake plugin added by xchatus_plugingui_add() */
	if (pl->fake)
		goto xit;
...
static xchatus_plugin *
plugin_list_add (xchatus_context *ctx, char *filename, const char *name,
					  const char *desc, const char *version, void *handle,
					  void *deinit_func, int fake, int free_strings)
{
	xchatus_plugin *pl;

	pl = g_new (xchatus_plugin, 1);
	pl->handle = handle;
...
#ifndef WIN32
static void *
xchatus_dummy (xchatus_plugin *ph)
{
	return NULL;
}
...
static int
xchatus_read_fd (xchatus_plugin *ph, GIOChannel *source, char *buf, int *len)
{
	GError *error = NULL;
				...
				void *deinit_func, char *arg, int fake)
{
	...
	if (!fake)
	{
		...
		pl->xchatus_hook_command = xchatus_hook_command;
		pl->xchatus_hook_server = xchatus_hook_server;
		pl->xchatus_hook_print = xchatus_hook_print;
		pl->xchatus_hook_timer = xchatus_hook_timer;
		pl->xchatus_hook_fd = xchatus_hook_fd;
		pl->xchatus_unhook = xchatus_unhook;
		pl->xchatus_print = xchatus_print;
		pl->xchatus_printf = xchatus_printf;
		pl->xchatus_command = xchatus_command;
		pl->xchatus_commandf = xchatus_commandf;
		pl->xchatus_nickcmp = xchatus_nickcmp;
		pl->xchatus_set_context = xchatus_set_context;
		pl->xchatus_find_context = xchatus_find_context;
		pl->xchatus_get_context = xchatus_get_context;
		pl->xchatus_get_info = xchatus_get_info;
		pl->xchatus_get_prefs = xchatus_get_prefs;
		pl->xchatus_list_get = xchatus_list_get;
		pl->xchatus_list_free = xchatus_list_free;
		pl->xchatus_list_fields = xchatus_list_fields;
		pl->xchatus_list_str = xchatus_list_str;
		pl->xchatus_list_next = xchatus_list_next;
		pl->xchatus_list_int = xchatus_list_int;
		pl->xchatus_plugingui_add = xchatus_plugingui_add;
		pl->xchatus_plugingui_remove = xchatus_plugingui_remove;
		pl->xchatus_emit_print = xchatus_emit_print;
#ifdef WIN32
		pl->xchatus_read_fd = (void *) xchatus_read_fd;
#else
		pl->xchatus_read_fd = xchatus_dummy;
#endif
		pl->xchatus_list_time = xchatus_list_time;
		pl->xchatus_gettext = xchatus_gettext;
		pl->xchatus_send_modes = xchatus_send_modes;
		pl->xchatus_strip = xchatus_strip;
		pl->xchatus_free = xchatus_free;
		pl->xchatus_pluginpref_set_str = xchatus_pluginpref_set_str;
		pl->xchatus_pluginpref_get_str = xchatus_pluginpref_get_str;
		pl->xchatus_pluginpref_set_int = xchatus_pluginpref_set_int;
		pl->xchatus_pluginpref_get_int = xchatus_pluginpref_get_int;
		pl->xchatus_pluginpref_delete = xchatus_pluginpref_delete;
		pl->xchatus_pluginpref_list = xchatus_pluginpref_list;
		pl->xchatus_hook_server_attrs = xchatus_hook_server_attrs;
		pl->xchatus_hook_print_attrs = xchatus_hook_print_attrs;
		pl->xchatus_emit_print_attrs = xchatus_emit_print_attrs;
		pl->xchatus_event_attrs_create = xchatus_event_attrs_create;
		pl->xchatus_event_attrs_free = xchatus_event_attrs_free;

		/* run xchatus_plugin_init, if it returns 0, close the plugin */
		if (((xchatus_init_func *)init_func) (pl, &pl->name, &pl->desc, &pl->version, arg) == 0)
		{
...
plugin_load (session *sess, char *filename, char *arg)
{
	...
	xchatus_init_func *init_func;
	xchatus_deinit_func *deinit_func;

	if (handle == NULL)
		return (char *)g_module_error ();

	/* find the init routine xchatus_plugin_init */
	if (!g_module_symbol (handle, "xchatus_plugin_init", (gpointer *)&init_func))
	{
		g_module_close (handle);
		return _("No xchatus_plugin_init symbol; is this really a XCHATUS plugin?");
	}

	/* find the plugin's deinit routine, if any */
	if (!g_module_symbol (handle, "xchatus_plugin_deinit", (gpointer *)&deinit_func))
		deinit_func = NULL;
...
plugin_hook_run (session *sess, char *name, char *word[], char *word_eol[],
				 xchatus_event_attrs *attrs, int type)
{
	GSList *list, *next;
	xchatus_hook *hook;
	int ret, eat = 0;
	...
	while (1)
	{
		...
		{
		case HOOK_COMMAND:
			ret = ((xchatus_cmd_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_PRINT_ATTRS:
			ret = ((xchatus_print_attrs_cb *)hook->callback) (word, attrs, hook->userdata);
			break;
		case HOOK_SERVER:
			ret = ((xchatus_serv_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_SERVER_ATTRS:
			ret = ((xchatus_serv_attrs_cb *)hook->callback) (word, word_eol, attrs, hook->userdata);
			break;
		default: /*case HOOK_PRINT:*/
			ret = ((xchatus_print_cb *)hook->callback) (word, hook->userdata);
			break;
		}
...
xchatus_event_attrs *
xchatus_event_attrs_create (xchatus_plugin *ph)
{
	return g_new0 (xchatus_event_attrs, 1);
}

void
xchatus_event_attrs_free (xchatus_plugin *ph, xchatus_event_attrs *attrs)
{
	g_free (attrs);
}
...
static int
plugin_timeout_cb (xchatus_hook *hook)
{
	int ret;
	...
	ret = ((xchatus_timer_cb *)hook->callback) (hook->userdata);
...
static void
plugin_insert_hook (xchatus_hook *new_hook)
{
	GSList *list;
	xchatus_hook *hook;
	int new_hook_type;
...
static gboolean
plugin_fd_cb (GIOChannel *source, GIOCondition condition, xchatus_hook *hook)
{
	int flags = 0, ret;
	typedef int (xchatus_fd_cb2) (int fd, int flags, void *user_data, GIOChannel *);
	...
	ret = ((xchatus_fd_cb2 *)hook->callback) (hook->pri, flags, hook->userdata, source);
...
static xchatus_hook *
plugin_add_hook (xchatus_plugin *pl, int type, int pri, const char *name,
					  const  char *help_text, void *callb, int timeout, void *userdata)
{
	xchatus_hook *hook;

	hook = g_new0 (xchatus_hook, 1);
	hook->type = type;
	hook->pri = pri;
...
void *
xchatus_unhook (xchatus_plugin *ph, xchatus_hook *hook)
{
	/* perl.c trips this */
	if (!g_slist_find (hook_list, hook) || hook->type == HOOK_DELETED)
		return NULL;
...
xchatus_hook *
xchatus_hook_command (xchatus_plugin *ph, const char *name, int pri,
						  xchatus_cmd_cb *callb, const char *help_text, void *userdata)
{
...
xchatus_hook *
xchatus_hook_server (xchatus_plugin *ph, const char *name, int pri,
						 xchatus_serv_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_SERVER, pri, name, 0, callb, 0, userdata);
}

xchatus_hook *
xchatus_hook_server_attrs (xchatus_plugin *ph, const char *name, int pri,
						   xchatus_serv_attrs_cb *callb, void *userdata)
{
...
xchatus_hook *
xchatus_hook_print (xchatus_plugin *ph, const char *name, int pri,
						xchatus_print_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_PRINT, pri, name, 0, callb, 0, userdata);
}

xchatus_hook *
xchatus_hook_print_attrs (xchatus_plugin *ph, const char *name, int pri,
						  xchatus_print_attrs_cb *callb, void *userdata)
{
...
xchatus_hook *
xchatus_hook_timer (xchatus_plugin *ph, int timeout, xchatus_timer_cb *callb,
					   void *userdata)
{
...
xchatus_hook *
xchatus_hook_fd (xchatus_plugin *ph, int fd, int flags,
					xchatus_fd_cb *callb, void *userdata)
{
	xchatus_hook *hook;
...
void
xchatus_print (xchatus_plugin *ph, const char *text)
{
	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\txchatus_print called without a valid context.\n", ph->name));
		return;
	}
...
void
xchatus_printf (xchatus_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	xchatus_print (ph, buf);
	g_free (buf);
}

void
xchatus_command (xchatus_plugin *ph, const char *command)
{
	char *command_utf8;

	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\txchatus_command called without a valid context.\n", ph->name));
		return;
	}
...
void
xchatus_commandf (xchatus_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	xchatus_command (ph, buf);
	g_free (buf);
}

int
xchatus_nickcmp (xchatus_plugin *ph, const char *s1, const char *s2)
{
...
int
xchatus_set_context (xchatus_plugin *ph, xchatus_context *context)
{
...
xchatus_context *
xchatus_find_context (xchatus_plugin *ph, const char *servname, const char *channel)
{
	return plugin_find_context (servname, channel, ph->context->server);
}

const char *
xchatus_get_info (xchatus_plugin *ph, const char *id)
{
	session *sess;
	guint32 hash;
	...
	if (!is_session (sess))
	{
		DEBUG(PrintTextf(0, "%s\txchatus_get_info called without a valid context.\n", ph->name));
		return NULL;
	}
...
int
xchatus_get_prefs (xchatus_plugin *ph, const char *name, const char **string, int *integer)
{
	int i = 0;
...
xchatus_list *
xchatus_list_get (xchatus_plugin *ph, const char *name)
{
	xchatus_list *list;

	list = g_new0 (xchatus_list, 1);
...
void
xchatus_list_free (xchatus_plugin *ph, xchatus_list *xlist)
{
...
int
xchatus_list_next (xchatus_plugin *ph, xchatus_list *xlist)
{
	if (xlist->next == NULL)
		return 0;
...
const char * const *
xchatus_list_fields (xchatus_plugin *ph, const char *name)
{
...
time_t
xchatus_list_time (xchatus_plugin *ph, xchatus_list *xlist, const char *name)
{
...
const char *
xchatus_list_str (xchatus_plugin *ph, xchatus_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
...
int
xchatus_list_int (xchatus_plugin *ph, xchatus_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
	gpointer data = ph->context;
...
void *
xchatus_plugingui_add (xchatus_plugin *ph, const char *filename,
...
void
xchatus_plugingui_remove (xchatus_plugin *ph, void *handle)
{
#ifdef USE_PLUGIN
...
int
xchatus_emit_print (xchatus_plugin *ph, const char *event_name, ...)
{
	va_list args;
...
char *
xchatus_gettext (xchatus_plugin *ph, const char *msgid)
{
	/* so that plugins can use XCHATUS's internal gettext strings. */
	/* e.g. The EXEC plugin uses this on Windows. */
	return _(msgid);
}

void
xchatus_send_modes (xchatus_plugin *ph, const char **targets, int ntargets, int modes_per_line, char sign, char mode)
{
...
char *
xchatus_strip (xchatus_plugin *ph, const char *str, int len, int flags)
{
	return strip_color ((char *)str, len, flags);
}

void
xchatus_free (xchatus_plugin *ph, void *ptr)
{
	g_free (ptr);
}

static int
xchatus_pluginpref_set_str_real (xchatus_plugin *pl, const char *var, const char *value, int mode) /* mode: 0 = delete, 1 = save */
{
	FILE *fpIn;
	int fhOut;
	int prevSetting;
	...
	fhOut = xchatus_open_file (confname_tmp, O_TRUNC | O_WRONLY | O_CREAT, 0600, XOF_DOMODE);
	fpIn = xchatus_fopen_file (confname, "r", 0);
...
int
xchatus_pluginpref_set_str (xchatus_plugin *pl, const char *var, const char *value)
{
	return xchatus_pluginpref_set_str_real (pl, var, value, 1);
}

static int
xchatus_pluginpref_get_str_real (xchatus_plugin *pl, const char *var, char *dest, int dest_len)
{
...
int
xchatus_pluginpref_get_str (xchatus_plugin *pl, const char *var, char *dest)
{
	/* All users of this must ensure dest is >= 512... */
	return xchatus_pluginpref_get_str_real (pl, var, dest, 512);
}

int
xchatus_pluginpref_set_int (xchatus_plugin *pl, const char *var, int value)
{
	char buffer[12];

	g_snprintf (buffer, sizeof (buffer), "%d", value);
	return xchatus_pluginpref_set_str_real (pl, var, buffer, 1);
}

int
xchatus_pluginpref_get_int (xchatus_plugin *pl, const char *var)
{
	char buffer[12];

	if (xchatus_pluginpref_get_str_real (pl, var, buffer, sizeof(buffer)))
	{
		int ret = atoi (buffer);
...
int
xchatus_pluginpref_delete (xchatus_plugin *pl, const char *var)
{
	return xchatus_pluginpref_set_str_real (pl, var, 0, 0);
}

int
xchatus_pluginpref_list (xchatus_plugin *pl, char* dest)
{
	FILE *fpIn;
	char confname[64];
