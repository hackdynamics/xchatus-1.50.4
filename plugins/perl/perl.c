...
perl_auto_load (void *unused)
{
	...
	xdir = xchatus_get_info (ph, "configdir");

	/* don't pollute the filesystem with script files, this only causes misuse of the folders
	 * only use ~/.config/xchatus/addons/ and %APPDATA%\XCHATUS\addons */
...
typedef struct
{
	SV *callback;
	SV *userdata;
	xchatus_hook *hook;   /* required for timers */
	xchatus_context *ctx; /* allow timers to remember their context */
...
execute_perl (SV * function, char *args)
{
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		xchatus_printf(ph, "Perl error: %s\n", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		/* remove undef from the top of the stack */
	} else if (count != 1) {
		xchatus_printf (ph, "Perl error: expected 1 value from %s, "
...
static SV *
list_item_to_sv ( xchatus_list *list, const char *const *fields )
{
	HV *hash = newHV();
	SV *field_value;
	const char *field;
	...
	while (fields[field_index] != NULL) {
		...
		case 's':
			field = xchatus_list_str (ph, list, field_name);
			if (field != NULL) {
			...
			break;
		case 'p':
			field_value = newSViv (PTR2IV (xchatus_list_str (ph, list,
																	 field_name)));
			break;
		case 'i':
			field_value = newSVuv (xchatus_list_int (ph, list, field_name));
			break;
		case 't':
			 ...
			 */
			field_value = newSVnv ((const NV) xchatus_list_time (ph, list, field_name));
			break;
		default:
			field_value = &PL_sv_undef;
		}
...
/* sets $XCHATUS::Embed::current_package */
static void
set_current_package (SV *package)
{
	SV *current_package = get_sv ("XCHATUS::Embed::current_package", 1);
...
fd_cb (int fd, int flags, void *userdata)
{
	...
	if (SvTRUE (ERRSV)) {
		xchatus_printf (ph, "Error in fd callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_ALL;
	} else {
		if (count != 1) {
			xchatus_print (ph, "Fd handler should only return 1 value.");
...
timer_cb (void *userdata)
{
	...
	if (data->ctx) {
		xchatus_set_context (ph, data->ctx);
	}
	...
	if (SvTRUE (ERRSV)) {
		xchatus_printf (ph, "Error in timer callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_ALL;
	} else {
		if (count != 1) {
			xchatus_print (ph, "Timer handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
			if (retVal == 0) {
...
server_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		xchatus_printf (ph, "Error in server callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_NONE;
	} else {
		if (count != 1) {
			xchatus_print (ph, "Server handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
		}
...
command_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	/*               xchatus_printf (ph, "Received %d words in command callback", */
	/*                               av_len (wd)); */
	PUSHMARK (SP);
	XPUSHs (newRV_noinc ((SV *) array2av (word)));
	XPUSHs (newRV_noinc ((SV *) array2av (word_eol)));
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		xchatus_printf (ph, "Error in command callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_HEXCHAT;
	} else {
		if (count != 1) {
			xchatus_print (ph, "Command handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
		}
...
print_cb (char *word[], void *userdata)
{
	...
	/*xchatus_printf (ph, "Received %d words in print callback", av_len (wd)+1); */
	PUSHMARK (SP);
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		xchatus_printf (ph, "Error in print callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_NONE;
	} else {
		if (count != 1) {
			xchatus_print (ph, "Print handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
		}
...
/* XCHATUS::Internal::register (scriptname, version, desc, shutdowncallback, filename)
 *
 */

static
XS (XS_XCHATUS_register)
{
	...
	if (items != 4) {
		xchatus_printf (ph,
						  "Usage: XCHATUS::Internal::register(scriptname, version, desc, filename)");
	} else {
		...
		gui_entry = xchatus_plugingui_add (ph, filename, name,
													desc, version, NULL);
...
XS (XS_XCHATUS_print)
{
	...
	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::Internal::print(text)");
	} else {
		text = SvPV_nolen (ST (0));
		xchatus_print (ph, text);
	}
	XSRETURN_EMPTY;
}

static
XS (XS_XCHATUS_emit_print)
{
	char *event_name;
	int RETVAL;
	int count;
	...
	dXSARGS;
	if (items < 1) {
		xchatus_print (ph, "Usage: XCHATUS::emit_print(event_name, ...)");
	} else {
		...
		switch (count) {
		case 1:
			RETVAL = xchatus_emit_print (ph, event_name, NULL);
			break;
		case 2:
			RETVAL = xchatus_emit_print (ph, event_name,
...
XS (XS_XCHATUS_send_modes)
{
	...
	if (items < 3 || items > 4) {
		xchatus_print (ph,
			"Usage: XCHATUS::send_modes( targets, sign, mode, modes_per_line)"
		);
	} else {
		if (SvROK (ST (0))) {
		...
		xchatus_send_modes (ph, targets, target_count, modes_per_line, sign, mode);
		g_free ((char**) targets);
	}
}
static
XS (XS_XCHATUS_get_info)
{
	SV *temp = NULL;
	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::get_info(id)");
	} else {
		SV *id = ST (0);
		const char *RETVAL;

		RETVAL = xchatus_get_info (ph, SvPV_nolen (id));
		if (RETVAL == NULL) {
			XSRETURN_UNDEF;
		}
...
static
XS (XS_XCHATUS_context_info)
{
	const char *const *fields;
	dXSARGS;

	if (items > 0 ) {
		xchatus_print (ph, "Usage: XCHATUS::Internal::context_info()");
	}
	fields = xchatus_list_fields (ph, "channels" );
	XPUSHs (list_item_to_sv (NULL, fields));
	XSRETURN (1);
}

static
XS (XS_XCHATUS_get_prefs)
{
	const char *str;
	int integer;
	SV *temp = NULL;
	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::get_prefs(name)");
	} else {


		switch (xchatus_get_prefs (ph, SvPV_nolen (ST (0)), &str, &integer)) {
		case 0:
			XSRETURN_UNDEF;
			break;
		case 1:
...
/* XCHATUS::Internal::hook_server(name, priority, callback, userdata) */
static
XS (XS_XCHATUS_hook_server)
{
	...
	if (items != 5) {
		xchatus_print (ph,
						 "Usage: XCHATUS::Internal::hook_server(name, priority, callback, userdata, package)");
	} else {
		name = SvPV_nolen (ST (0));
		pri = (int) SvIV (ST (1));
		callback = ST (2);
		...
		hook = xchatus_hook_server (ph, name, pri, server_cb, data);

		XSRETURN_IV (PTR2IV (hook));
	}
}

/* XCHATUS::Internal::hook_command(name, priority, callback, help_text, userdata) */
static
XS (XS_XCHATUS_hook_command)
{
	char *name;
	int pri;
	SV *callback;
	...
	if (items != 6) {
		xchatus_print (ph,
						 "Usage: XCHATUS::Internal::hook_command(name, priority, callback, help_text, userdata, package)");
	} else {
		name = SvPV_nolen (ST (0));
		pri = (int) SvIV (ST (1));
		callback = ST (2);
		...
		data->depth = 0;
		data->package = newSVsv (package);
		hook = xchatus_hook_command (ph, name, pri, command_cb, help_text, data);
...
/* XCHATUS::Internal::hook_print(name, priority, callback, [userdata]) */
static
XS (XS_XCHATUS_hook_print)
{
	...
	if (items != 5) {
		xchatus_print (ph,
						 "Usage: XCHATUS::Internal::hook_print(name, priority, callback, userdata, package)");
	} else {
		name = SvPV_nolen (ST (0));
		pri = (int) SvIV (ST (1));
		callback = ST (2);
		...
		data->depth = 0;
		data->package = newSVsv (package);
		hook = xchatus_hook_print (ph, name, pri, print_cb, data);

		XSRETURN_IV (PTR2IV (hook));
	}
}

/* XCHATUS::Internal::hook_timer(timeout, callback, userdata) */
static
XS (XS_XCHATUS_hook_timer)
{
	int timeout;
	SV *callback;
	SV *userdata;
	...
	if (items != 4) {
		xchatus_print (ph,
						 "Usage: XCHATUS::Internal::hook_timer(timeout, callback, userdata, package)");
	} else {
		...
		data->callback = newSVsv (callback);
		data->userdata = newSVsv (userdata);
		data->ctx = xchatus_get_context (ph);
		data->package = newSVsv (package);
		hook = xchatus_hook_timer (ph, timeout, timer_cb, data);
		data->hook = hook;
...
/* XCHATUS::Internal::hook_fd(fd, callback, flags, userdata) */
static
XS (XS_XCHATUS_hook_fd)
{
	int fd;
	SV *callback;
	int flags;
	...
	if (items != 5) {
		xchatus_print (ph,
						 "Usage: XCHATUS::Internal::hook_fd(fd, callback, flags, userdata)");
	} else {
		fd = (int) SvIV (ST (0));
		callback = ST (1);
		flags = (int) SvIV (ST (2));
		...
		if ((flags & HEXCHAT_FD_NOTSOCKET) == 0) {
			...
			if (fd < 0) {
				xchatus_print(ph, "Invalid file descriptor");
				XSRETURN_UNDEF;
			}
		}
#endif
		...
		data->depth = 0;
		data->package = newSVsv (package);
		hook = xchatus_hook_fd (ph, fd, flags, fd_cb, data);
		data->hook = hook;
...
XS (XS_XCHATUS_unhook)
{
	...
	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::unhook(hook)");
	} else {
		hook = INT2PTR (xchatus_hook *, SvUV (ST (0)));
		userdata = (HookData *) xchatus_unhook (ph, hook);
...
/* XCHATUS::Internal::command(command) */
static
XS (XS_XCHATUS_command)
{
	char *cmd = NULL;

	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::Internal::command(command)");
	} else {
		cmd = SvPV_nolen (ST (0));
		xchatus_command (ph, cmd);
...
static
XS (XS_XCHATUS_find_context)
{
	char *server = NULL;
	char *chan = NULL;
	xchatus_context *RETVAL;

	dXSARGS;
	if (items > 2)
		xchatus_print (ph, "Usage: XCHATUS::find_context ([channel, [server]])");
	{
		...
		case 1:						  /* channel name only */
			...
			if (SvTRUE (ST (0)) || SvNIOK (ST (0))) {
				...
				/*                               xchatus_printf( ph, "XSUB - find_context( %s, NULL )", chan ); */
			}
			/* else { xchatus_print( ph, "XSUB - find_context( NULL, NULL )" ); } */
		...
		case 2:						  /* server and channel */
			...
			if (SvTRUE (ST (0)) || SvNIOK (ST (0))) {
				chan = SvPV_nolen (ST (0));
				/*                               xchatus_printf( ph, "XSUB - find_context( %s, NULL )", SvPV_nolen(ST(0) )); */
			}

			/* else { xchatus_print( ph, "XSUB - 2 arg NULL chan" ); } */
			/* change server value only if it is true or 0 */
			/* otherwise leave it as NULL */
			if (SvTRUE (ST (1)) || SvNIOK (ST (1))) {
				server = SvPV_nolen (ST (1));
				/*                               xchatus_printf( ph, "XSUB - find_context( NULL, %s )", SvPV_nolen(ST(1) )); */
			}
			/*  else { xchatus_print( ph, "XSUB - 2 arg NULL server" ); } */
			break;
		}

		RETVAL = xchatus_find_context (ph, server, chan);
		if (RETVAL != NULL) {
			/*                      xchatus_print (ph, "XSUB - context found"); */
			XSRETURN_IV (PTR2IV (RETVAL));
		} else {
			/*           xchatus_print (ph, "XSUB - context not found"); */
			XSRETURN_UNDEF;
		}
	}
}
...
static
XS (XS_XCHATUS_get_context)
{
	dXSARGS;
	if (items != 0) {
		xchatus_print (ph, "Usage: XCHATUS::get_context()");
	} else {
		XSRETURN_IV (PTR2IV (xchatus_get_context (ph)));
	}
}

static
XS (XS_XCHATUS_set_context)
{
	xchatus_context *ctx;
	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::set_context(ctx)");
	} else {
		ctx = INT2PTR (xchatus_context *, SvUV (ST (0)));
		XSRETURN_IV ((IV) xchatus_set_context (ph, ctx));
	}
}

static
XS (XS_XCHATUS_nickcmp)
{
	dXSARGS;
	if (items != 2) {
		xchatus_print (ph, "Usage: XCHATUS::nickcmp(s1, s2)");
	} else {
		XSRETURN_IV ((IV) xchatus_nickcmp (ph, SvPV_nolen (ST (0)),
													SvPV_nolen (ST (1))));
	}
}

static
XS (XS_XCHATUS_get_list)
{
	SV *name;
	xchatus_list *list;
	const char *const *fields;
	...
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::get_list(name)");
	} else {
		SP -= items;				  /*remove the argument list from the stack */

		name = ST (0);

		list = xchatus_list_get (ph, SvPV_nolen (name));
		...
		if (GIMME_V == G_SCALAR) {
			while (xchatus_list_next (ph, list)) {
				count++;
			}
		...
		fields = xchatus_list_fields (ph, SvPV_nolen (name));
		while (xchatus_list_next (ph, list)) {
...
static
XS (XS_XCHATUS_Embed_plugingui_remove)
{
	void *gui_entry;
	dXSARGS;
	if (items != 1) {
		xchatus_print (ph, "Usage: XCHATUS::Embed::plugingui_remove(handle)");
	} else {
		gui_entry = INT2PTR (void *, SvUV (ST (0)));
		xchatus_plugingui_remove (ph, gui_entry);
	}
	XSRETURN_EMPTY;
}

static
XS (XS_XCHATUS_plugin_pref_set)
{
	dMARK;
	dAX;

	XSRETURN_IV ((IV) xchatus_pluginpref_set_str (ph, SvPV_nolen (ST (0)),
													SvPV_nolen (ST (1))));
}

static
XS (XS_XCHATUS_plugin_pref_get)
{
	int result;
	char value[512];
	...
	result = xchatus_pluginpref_get_str (ph, SvPV_nolen (ST (0)), value);
...
static
XS (XS_XCHATUS_plugin_pref_delete)
{
	dMARK;
	dAX;
	
	XSRETURN_IV ((IV) xchatus_pluginpref_delete (ph, SvPV_nolen (ST (0))));
}

static
XS (XS_XCHATUS_plugin_pref_list)
{
	char list[4096];
	char value[512];
	char *token;
	...
	if (!xchatus_pluginpref_list (ph, list))
		XSRETURN_EMPTY;
	...
	while (token != NULL)
	{
		xchatus_pluginpref_get_str (ph, token, value);
...
xs_init (pTHX)
{
	...
	newXS ("XCHATUS::Internal::register", XS_XCHATUS_register, __FILE__);
	newXS ("XCHATUS::Internal::hook_server", XS_XCHATUS_hook_server, __FILE__);
	newXS ("XCHATUS::Internal::hook_command", XS_XCHATUS_hook_command, __FILE__);
	newXS ("XCHATUS::Internal::hook_print", XS_XCHATUS_hook_print, __FILE__);
	newXS ("XCHATUS::Internal::hook_timer", XS_XCHATUS_hook_timer, __FILE__);
	newXS ("XCHATUS::Internal::hook_fd", XS_XCHATUS_hook_fd, __FILE__);
	newXS ("XCHATUS::Internal::unhook", XS_XCHATUS_unhook, __FILE__);
	newXS ("XCHATUS::Internal::print", XS_XCHATUS_print, __FILE__);
	newXS ("XCHATUS::Internal::command", XS_XCHATUS_command, __FILE__);
	newXS ("XCHATUS::Internal::set_context", XS_XCHATUS_set_context, __FILE__);
	newXS ("XCHATUS::Internal::get_info", XS_XCHATUS_get_info, __FILE__);
	newXS ("XCHATUS::Internal::context_info", XS_XCHATUS_context_info, __FILE__);
	newXS ("XCHATUS::Internal::get_list", XS_XCHATUS_get_list, __FILE__);

	newXS ("XCHATUS::Internal::plugin_pref_set", XS_XCHATUS_plugin_pref_set, __FILE__);
	newXS ("XCHATUS::Internal::plugin_pref_get", XS_XCHATUS_plugin_pref_get, __FILE__);
	newXS ("XCHATUS::Internal::plugin_pref_delete", XS_XCHATUS_plugin_pref_delete, __FILE__);
	newXS ("XCHATUS::Internal::plugin_pref_list", XS_XCHATUS_plugin_pref_list, __FILE__);
	
	newXS ("XCHATUS::find_context", XS_XCHATUS_find_context, __FILE__);
	newXS ("XCHATUS::get_context", XS_XCHATUS_get_context, __FILE__);
	newXS ("XCHATUS::get_prefs", XS_XCHATUS_get_prefs, __FILE__);
	newXS ("XCHATUS::emit_print", XS_XCHATUS_emit_print, __FILE__);
	newXS ("XCHATUS::send_modes", XS_XCHATUS_send_modes, __FILE__);
	newXS ("XCHATUS::nickcmp", XS_XCHATUS_nickcmp, __FILE__);

	newXS ("XCHATUS::Embed::plugingui_remove", XS_XCHATUS_Embed_plugingui_remove,
			 __FILE__);

	stash = get_hv ("XCHATUS::", TRUE);
	if (stash == NULL) {
		exit (1);
	}
	...
	version = get_sv( "XCHATUS::VERSION", 1 );
...
perl_init (void)
{
	...
	static const char xchat_definitions[] = {
		/* Redefine the $SIG{__WARN__} handler to have XCHATUS
		   printing warnings in the main window. (TheHobbit) */
#include "xchatus.pm.h"
	};
#ifdef OLD_PERL
...
#include "irc.pm.h"
	...
	warn = 0;
	xchatus_get_prefs (ph, "perl_warnings", NULL, &warn);
	arg_count = warn ? 4 : 3;
...
perl_load_file (char *filename)
{
			...
			if (GetLastError () == ERROR_BAD_EXE_FORMAT)
...
#ifdef _WIN64
								 "64-bit XCHATUS Perl is required.");
#else
								 "32-bit XCHATUS Perl is required.");
#endif
			else {
				...
				if (lib) {
					...
					thread_mbox ("Cannot open " PERL_DLL "!\n\n"
									 ...
									 "http://xchatus.github.io/downloads.html\n\n"
	...
	return execute_perl (sv_2mortal (newSVpv ("XCHATUS::Embed::load", 0)),
								filename);
...
perl_end (void)
{
	...
	if (my_perl != NULL) {
		execute_perl (sv_2mortal (newSVpv ("XCHATUS::Embed::unload_all", 0)), "");
...
{
	if (my_perl != NULL) {
		execute_perl (sv_2mortal (newSVpv ("XCHATUS::Embed::reload_all", 0)), "");
...
perl_command_unload (char *word[], char *word_eol[], void *userdata)
{
	...
	if (my_perl != NULL && file != NULL) {
		execute_perl (sv_2mortal (newSVpv ("XCHATUS::Embed::unload", 0)), file);
		return HEXCHAT_EAT_HEXCHAT;
	}
...
perl_command_reload (char *word[], char *word_eol[], void *eat)
{
	...
	if (my_perl != NULL && file != NULL) {
		execute_perl (sv_2mortal (newSVpv ("XCHATUS::Embed::reload", 0)), file);
		return HEXCHAT_EAT_HEXCHAT;
	}
...
{
	if (my_perl != NULL)
		execute_perl (sv_2mortal (newSVpv ("XCHATUS::Embed::evaluate", 0)), word_eol[2]);

	return HEXCHAT_EAT_HEXCHAT;
}

void
xchatus_plugin_get_info (char **name, char **desc, char **version,
							  void **reserved)
{
	*name = "Perl";
...
int
xchatus_plugin_init (xchatus_plugin * plugin_handle, char **plugin_name,
						 char **plugin_desc, char **plugin_version, char *arg)
{
	if (initialized != 0) {
		xchatus_print (plugin_handle, "Perl interface already loaded\n");
		return 0;
	}
	...
	*plugin_version = PACKAGE_VERSION;
	...
	xchatus_hook_command (ph, "load", HEXCHAT_PRI_NORM, perl_command_load, 0, 0);
	xchatus_hook_command (ph, "unload", HEXCHAT_PRI_NORM, perl_command_unload, 0,
							  0);
	xchatus_hook_command (ph, "reload", HEXCHAT_PRI_NORM, perl_command_reload, 0,
							  0);
	xchatus_hook_command (ph, "pl_reload", HEXCHAT_PRI_NORM, perl_command_reload,
							  "Reloads a Perl script. Syntax: /pl_reload <filename.pl>", (int*)1);
	xchatus_hook_command (ph, "unloadall", HEXCHAT_PRI_NORM,
							  perl_command_unloadall, "Unloads all loaded Perl scripts.", 0);
	xchatus_hook_command (ph, "reloadall", HEXCHAT_PRI_NORM,
							  perl_command_reloadall, "Realoads all loaded Perl scripts.", 0);

	xchatus_hook_command (ph, "pl", HEXCHAT_PRI_NORM,
							  perl_command_eval, "Evaluates Perl code. Syntax: /pl <perl code>", 0);

	/*perl_init (); */
	xchatus_hook_timer (ph, 0, perl_auto_load, NULL );

	xchatus_print (ph, "Perl interface loaded\n");

	return 1;
}

int
xchatus_plugin_deinit (xchatus_plugin * plugin_handle)
{
	perl_end ();

	initialized = 0;
	xchatus_print (plugin_handle, "Perl interface unloaded\n");
