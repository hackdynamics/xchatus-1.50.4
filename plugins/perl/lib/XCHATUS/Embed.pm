...
#   gui_entry
#     This is xchatus_plugin pointer that is used to remove the script from
...
sub unload {
	...
	if( $pkg_info ) {	
		...
		return XCHATUS::EAT_ALL;
	} else {
		XCHATUS::print( qq{"$file" is not loaded.\n} );
		return XCHATUS::EAT_NONE;
	}
}
...
sub reload_all {
	my @dirs = XCHATUS::get_info( "configdir" );
...
sub evaluate {
	...
	XCHATUS::print $@ if $@; #print warnings

	local $Data::Dumper::Sortkeys = 1;
	local $Data::Dumper::Terse    = 1;

	if (@results > 1) {
		XCHATUS::print Dumper \@results;
	}
	elsif (ref $results[0] || !$results[0]) {
		XCHATUS::print Dumper $results[0];
	}
	else {
		XCHATUS::print $results[0];
	}

	return XCHATUS::EAT_HEXCHAT;
};
...
sub find_external_pkg {
	...
	while( my @frame = caller( $level ) ) {
		return @frame if $frame[0] !~ /(?:^IRC$|^XCHATUS)/;
		$level++;
	}
	return;
}
...
sub find_pkg {
	...
	while( my ($package, $file, $line) = caller( $level ) ) {
		return $package if $package =~ /^XCHATUS::Script::/;
		$level++;
	}
