$SIG{__WARN__} = sub {
	...
	if( $message =~ /^(?:Gtk|GLib|Gdk)(?:-\w+)?-$message_levels/i ) {
		...
		if( defined &XCHATUS::Internal::print ) {
			XCHATUS::print( $message );
		} else {
			warn $message;
		}
	}
...
sub get_context;
sub XCHATUS::Internal::context_info;
sub XCHATUS::Internal::print;
...
sub register {
	my ($package, $calling_package) = XCHATUS::Embed::find_pkg();
	my $pkg_info = XCHATUS::Embed::pkg_info( $package );
	my $filename = $pkg_info->{filename};
	my ($name, $version, $description, $callback) = @_;
	
	if( defined $pkg_info->{gui_entry} ) {
		XCHATUS::print( "XCHATUS::register called more than once in "
			. $pkg_info->{filename} );
		return ();
	}
	...
	if( $callback ) {
		$callback = XCHATUS::Embed::fix_callback(
	...
	$pkg_info->{gui_entry} =
		XCHATUS::Internal::register( $name, $version, $description, $filename );
...
sub hook_server {
	...
	my ($priority, $data) = ( XCHATUS::PRI_NORM, undef );
	...
	my $pkg_info = XCHATUS::Embed::pkg_info( $package );
	my $hook = XCHATUS::Internal::hook_server(
...
sub hook_command {
	...
	my ($priority, $help_text, $data) = ( XCHATUS::PRI_NORM, undef, undef );
	_process_hook_options(
		$options,
	...
	my $pkg_info = XCHATUS::Embed::pkg_info( $package );
	my $hook = XCHATUS::Internal::hook_command(
...
sub hook_print {
	...
	my ($priority, $run_after, $filter, $data) = ( XCHATUS::PRI_NORM, 0, 0, undef );
	_process_hook_options(
		$options,
	...
	if( $run_after and $filter ) {
		Carp::carp( "XCHATUS::hook_print's run_after_event and filter options are mutually exclusive, you can only use of them at a time per hook" );
		return;
	}
	...
	my $pkg_info = XCHATUS::Embed::pkg_info( $package );
	my $hook = XCHATUS::Internal::hook_print(
...
sub hook_timer {
	...
	my $pkg_info = XCHATUS::Embed::pkg_info( $package );
	my $hook = XCHATUS::Internal::hook_timer( $timeout, $callback, $data, $package );
	push @{$pkg_info->{hooks}}, $hook if defined $hook;
	return $hook;
}
...
sub hook_fd {
	...
	my ($flags, $data) = (XCHATUS::FD_READ, undef);
	_process_hook_options(
		$options,
		[qw(flags data)],
		[\($flags, $data)],
	...
	my $pkg_info = XCHATUS::Embed::pkg_info( $package );
	my $hook = XCHATUS::Internal::hook_fd(
		$fileno, $cb, $flags, {
...
sub unhook {
	...
	if( defined( $hook )
		...
		return XCHATUS::Internal::unhook( $hook );
	}
	return ();
}
...
sub _do_for_each {
	...
	my $num_done = 0;
	my $old_ctx = XCHATUS::get_context();
	for my $server ( @$servers ) {
		for my $channel ( @$channels ) {
			if( XCHATUS::set_context( $channel, $server ) ) {
				$cb->();
				$num_done++
			}
		}
	}
	XCHATUS::set_context( $old_ctx );
	return $num_done;
}
...
sub print {
	...
	return _do_for_each(
		sub { XCHATUS::Internal::print( $text ); },
		@_
	);
}
...
sub printf {
	my $format = shift;
	XCHATUS::print( sprintf( $format, @_ ) );
}

# make XCHATUS::prnt() and XCHATUS::prntf() as aliases for XCHATUS::print() and 
# XCHATUS::printf(), mainly useful when these functions are exported
sub prnt {
	goto &XCHATUS::print;
}
...
sub command {
	...
	return _do_for_each(
		sub { XCHATUS::Internal::command( $_ ) foreach @commands },
		@_
	);
}
...
sub commandf {
	my $format = shift;
	XCHATUS::command( sprintf( $format, @_ ) );
}
...
sub plugin_pref_set {
	...
	return XCHATUS::Internal::plugin_pref_set($setting, $value);
}

sub plugin_pref_get {
	my $setting = shift // return 0;

	return XCHATUS::Internal::plugin_pref_get($setting);
}

sub plugin_pref_delete {
	my $setting = shift // return 0;

	return XCHATUS::Internal::plugin_pref_delete($setting);
}

sub plugin_pref_list {
	my %list = XCHATUS::Internal::plugin_pref_list();
...
sub set_context {
	...
	if( @_ == 2 ) {
		my ($channel, $server) = @_;
		$context = XCHATUS::find_context( $channel, $server );
	} elsif( @_ == 1 ) {
		if( defined $_[0] && $_[0] =~ /^\d+$/ ) {
			$context = $_[0];
		} else {
			$context = XCHATUS::find_context( $_[0] );
		}
	} elsif( @_ == 0 ) {
		$context = XCHATUS::find_context();
	}
	return $context ? XCHATUS::Internal::set_context( $context ) : 0;
}
...
sub get_info {
	...
	if( defined( $id ) ) {
		if( grep { $id eq $_ } qw(state_cursor id) ) {
			$info = XCHATUS::get_prefs( $id );
		} else {
			$info = XCHATUS::Internal::get_info( $id );
		}
	}
	return $info;
}
...
sub user_info {
	my $nick = XCHATUS::strip_code(shift @_ || XCHATUS::get_info( "nick" ));
	my $user;
	for (XCHATUS::get_list( "users" ) ) {
		if ( XCHATUS::nickcmp( $_->{nick}, $nick ) == 0 ) {
			$user = $_;
			last;
		}
	}
...
sub context_info {
	my $ctx = shift @_ || XCHATUS::get_context;
	my $old_ctx = XCHATUS::get_context;
	my @fields = (
	...
	if( XCHATUS::set_context( $ctx ) ) {
		my %info;
		for my $field ( @fields ) {
			$info{$field} = XCHATUS::get_info( $field );
		}
		
		my $ctx_info = XCHATUS::Internal::context_info;
...
sub get_list {
	...
	if( $_[0] eq 'networks' ) {
		return XCHATUS::List::Network->get();
	} else {
		return XCHATUS::Internal::get_list( $_[0] );
	}
}
