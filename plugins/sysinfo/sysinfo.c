/*
 * SysInfo - sysinfo plugin for XCHATUS
...
#define _(x) xchatus_gettext(ph,x)
...
get_client (void)
{
	return g_strdup_printf ("XCHATUS %s", xchatus_get_info(ph, "version"));
}
...
print_summary (gboolean announce)
{
	...
	xchatus_commandf (ph, "%s %s", announce ? "SAY" : "ECHO", output);
...
print_info (char *info, gboolean announce)
{
	...
	for (i = 0; hwinfos[i].name != NULL; i++)
	{
		if (!g_ascii_strcasecmp (info, hwinfos[i].name))
		{
			...
			if (str)
			{
				xchatus_commandf (ph, "%s \002%s\002: %s", announce ? "SAY" : "ECHO",
									hwinfos[i].title, str);
				g_free (str);
			}
			else
				xchatus_print (ph, _("Sysinfo: Failed to get info. Either not supported or error."));
			return;
		}
	}

	xchatus_print (ph, _("Sysinfo: No info by that name\n"));
}

static gboolean
sysinfo_get_bool_pref (const char *pref, gboolean def)
{
	int value = xchatus_pluginpref_get_int (ph, pref);
...
sysinfo_set_pref_real (const char *pref, char *value, gboolean def)
{
	if (value && value[0])
	{
		...
		xchatus_pluginpref_set_int (ph, pref, i != 0);
		xchatus_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref, i != 0);
	}
	else
	{
		xchatus_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref,
...
sysinfo_set_pref (char *key, char *value)
{
	if (!key || !key[0])
	{
		xchatus_print (ph, _("Sysinfo: Valid settings are: announce and hide_* for each piece of information. e.g. hide_os. Without a value it will show current (or default) setting.\n"));
		return;
	}
	...
	xchatus_print (ph, _("Sysinfo: Invalid variable name\n"));
}
...
sysinfo_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	channel_type = xchatus_list_int (ph, NULL, "type");
...
int
xchatus_plugin_init (xchatus_plugin *plugin_handle, char **plugin_name, char **plugin_desc, char **plugin_version, char *arg)
{
	ph = plugin_handle;
	*plugin_name = name;
	*plugin_desc = desc;
	*plugin_version = version;
	...
	xchatus_hook_command (ph, "SYSINFO", HEXCHAT_PRI_NORM, sysinfo_cb, sysinfo_help, NULL);

	xchatus_command (ph, "MENU ADD \"Window/Send System Info\" \"SYSINFO\"");
	xchatus_printf (ph, _("%s plugin loaded\n"), name);
	return 1;
}

int
xchatus_plugin_deinit (void)
{
	xchatus_command (ph, "MENU DEL \"Window/Display System Info\"");
	xchatus_printf (ph, _("%s plugin unloaded\n"), name);
	return 1;
}
